...
//***********************************************************************/
	...
	function calcSun(longitude, latitude, timezone,
...
//*********************************************************************/
			...
			var JD = calcJD(year, indexRS, day);
			var dow = calcDayOfWeek(JD);
			var doy = calcDayOfYear(indexRS, day, isLeapYear(year));
			var T = calcTimeJulianCent(JD);
...
//*********************************************************************/
			...
			if (!nosunrise)		// Sunrise was found
			{
				var riseTimeLST = riseTimeGMT - (60 * zone) + daySavings;	
					//	in minutes
				var riseStr = timeString(riseTimeLST, JD);
				var utcRiseStr = timeString(riseTimeGMT, JD);

				print("sunrise : " + riseStr + "   [" + utcRiseStr + "]");
			}
			...
			print("noon    : " + solnStr + "   [" + utcSolnStr + "]");

			if (!nosunset)		// Sunset was found
			{
				var setTimeLST = setTimeGMT - (60 * zone) + daySavings;
				var setStr = timeString(setTimeLST, JD);
				var utcSetStr = timeString(setTimeGMT, JD);

				print("sunset  : " + setStr + "   [" + utcSetStr + "]");
			}
...
function getLatitude(dir, degs, mins, secs)

{
	...
	var dec = sign * (degs + (mins / 60) + (secs / 3600));
	return dec;

}	
// Returns decimal longitude from the deg, min, sec entered in the form	

function getLongitude(dir, degs, mins, secs)
...
var lodir = arguments[0];
var lodeg = arguments[1];
var lomin = arguments[2];
var losec = arguments[3];
var ladir = arguments[4];
var ladeg = arguments[5];
var lamin = arguments[6];
var lasec = arguments[7];
var tz = -arguments[8];
var day = arguments[9];
var month = arguments[10];
var year = arguments[11];
var hour = arguments[12];
var min = arguments[13];
var sec = arguments[14];

var latitude = getLatitude(ladir, ladeg, lamin, lasec)
var longitude = getLongitude(lodir, lodeg, lomin, losec);

print("longitude : " + lodir + " " + lodeg + " " + lomin + " " + losec + " = " + longitude);
print("latitude : " + ladir + " " + ladeg + " " + lamin + " " + lasec + " = " + latitude);
print("timezone : " + tz);
print("date : " + hour + ":" + min + ":" + sec + " " 
		+ day + "." + month + "." + year);
print();

calcSun(longitude, latitude,
		tz,
		year, month, day,
		hour, min, sec);
print();
